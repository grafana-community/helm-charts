...
hostAliases:
  {{- toYaml . | nindent 2 }}
{{- end }}
{{- if and (semverCompare ">=1.33-0" (include "loki.kubeVersion" .)) }}
...
containers:
  - name: {{ $target }}
    ...
    {{- with coalesce .Values.loki.command $component.command }}
    command:
      - {{ . | quote }}
    {{- end }}
    args:
      ...
      - -target={{ $target }}
      {{- with (concat .Values.global.extraArgs .Values.loki.extraArgs $component.extraArgs) | uniq }}
      {{- toYaml . | nindent 6 }}
      {{- end }}
    ports:
      - name: http-metrics
      ...
      - name: http-memberlist
        containerPort: 7946
        protocol: TCP
    {{- with (concat .Values.global.extraEnv .Values.loki.extraEnv $component.extraEnv) | uniq }}
    env:
      {{- toYaml . | nindent 6 }}
    {{- end }}
    {{- with (concat .Values.global.extraEnvFrom .Values.loki.extraEnvFrom $component.extraEnvFrom) | uniq }}
    envFrom:
      {{- toYaml . | nindent 6 }}
    {{- end }}
    {{- with .Values.loki.containerSecurityContext }}
    securityContext:
      {{- toYaml . | nindent 6 }}
    {{- end }}
