...
## Sidecar for library panels

If the parameter `sidecar.librarypanels.enabled` is set, a sidecar container is deployed in the Grafana
pod. This container watches ConfigMaps with the label defined in `sidecar.librarypanels.label`, writes
their `*.json` files to a local folder, and then uploads those JSON documents to Grafana's
`/api/library-elements` API.

By default the sidecar authenticates with the admin credentials generated by this chart and talks to
`http://localhost:3000`. If you disable the chart-managed admin user or prefer token-based auth,
provide `GRAFANA_TOKEN` or `GRAFANA_USER`/`GRAFANA_PASSWORD` through
`sidecar.librarypanels.env` or `sidecar.librarypanels.envValueFrom`.

The sync is idempotent for create/update operations and matches library panels by `uid`. Each JSON file
must therefore contain a `uid` field.

Optional delete reconciliation is available via `sidecar.librarypanels.deleteOnMissing: true`. When
enabled, the sidecar removes only UIDs it previously managed and that are now missing from watched JSON
files. Managed UIDs are tracked in `sidecar.librarypanels.stateFile` (default:
`/tmp/library-panels/.managed-uids.json`).

Example library panel config:

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: sample-grafana-library-panel
  labels:
    grafana_library_panel: "1"
data:
  stat-panel.json: |-
    {
      "uid": "cpu-summary",
